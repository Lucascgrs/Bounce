...
class Cercle:
    def __init__(self, position, rayon, couleur="black", epaisseur=2, life=20, angle_ouverture=360, angle_rotation=0, vitesse_rotation=0):
        ...
        self.angle_ouverture = angle_ouverture  # Angle d'ouverture en degrés (360 = cercle complet)
    ...
    def est_dans_ouverture(self, position_balle):
        """Vérifie si une position est dans l'ouverture de l'arc"""
        if self.angle_ouverture >= 360:
        ...
        # Calculer les angles de début et fin de l'ouverture
    ...
    def afficher(self, surface):
        ...
        if self.angle_ouverture >= 360:
            ...
            angle_debut_rad = math.radians(self.angle_rotation - self.angle_ouverture / 2)
            angle_fin_rad = math.radians(self.angle_rotation + self.angle_ouverture / 2)
            ...
            num_segments = max(10, int(self.angle_ouverture / 2))  # Plus de segments pour les gros arcs
            angle_step = (angle_fin_rad - angle_debut_rad) / num_segments

            for i in range(num_segments):
                angle1 = angle_debut_rad + i * angle_step
                angle2 = angle_debut_rad + (i + 1) * angle_step
            ...
            # Réduire et afficher
            arc_lisse = pygame.transform.smoothscale(temp_surface, (self.rayon * 2 + self.epaisseur * 2, self.rayon * 2 + self.epaisseur * 2))

            surface.blit(arc_lisse, (self.position[0] - self.rayon - self.epaisseur, self.position[1] - self.rayon - self.epaisseur))